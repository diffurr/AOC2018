...
(define-record-type Record (fields id minute elapsed))
...
(define make-Record-from-string
    (lambda (line)
        (let* 
            ...
            (make-Record id minute elapsed))))

(define parse
    (lambda (file)
        (let read-lines ((line (get-line file)) (records '()))
            (if (eof-object? line)
                records
                (read-lines (get-line file) (cons (make-Record-from-string line) records))))))
...
(define update
    ...
    (define up-table
        (lambda (proc table id begin record)
            ...
            (set-cdr! pair (mark-ttable (cdr pair) begin (Record-minute (car record))))
            (proc table id -1 (cdr record)))
        ))
...
(define test-parse
    (lambda ()
        (writeln "parse")
        (time (parse (open-input-file "input.txt")))
    ))
